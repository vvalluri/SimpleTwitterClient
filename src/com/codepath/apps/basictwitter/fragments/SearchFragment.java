...
public class SearchFragment extends TweetListFragment {
	...
	public static SearchFragment newInstance(String sname) {
		SearchFragment searchFragment = new SearchFragment();
        Bundle args = new Bundle();
        args.putString("search", sname);
	...
	public void onScrollPopulateTimeline (int count) {
		onScroll = true;
		onRefresh = false;
    	populateSearchResults(count);
		return;
	}
	
	protected void populateSearchResults(int totalItemsCount) {
	    	...
	    	if (onScroll) {
	    		Log.d("debug", "On scroll populate method invoked");
	    	...
	    	if (onRefresh) {
	    		Log.d("debug", "On refresh populate method invoked");
		    	if (local_sinceid == 0) {
	    	...
	    	Log.d("debug", "On User Timeline fetch: Max: " + local_maxid + "Since: " + firstSinceId);
			// Get the Search time line messages
			Client.getSearchTimeline(search, local_maxid, local_sinceid, new JsonHttpResponseHandler() {
				@Override
