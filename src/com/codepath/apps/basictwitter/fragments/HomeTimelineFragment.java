...
public class HomeTimelineFragment extends TweetListFragment {
	...
	public void onScrollPopulateTimeline (int count) {
		onScroll = true;
		onRefresh = false;
    	populateHomeTimeline(count);
		return;
	}
	
    protected void populateHomeTimeline(int totalItemsCount) {
    	...
    	if (isNetworkAvailable() == false) {
    	    ...
    	    if (dbTweets.isEmpty()) {
    	    	Log.d("debug", "No entries in persistence DB");
    	    	return;
    	    }
    	    // Populate DB entries into adapter
    	...
    	if (onScroll) {
    		Log.d("debug", "Hometimeline On scroll populate method invoked");
    	...
    	if (onRefresh) {
    		Log.d("debug", "Hometimeline On refresh populate method invoked");
	    	if (local_sinceid == 0) {
		...
		Client.getHomeTimeLineNext(local_maxid, local_sinceid, new JsonHttpResponseHandler() {
			@Override
